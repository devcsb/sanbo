...
원본으로 삼고 샘플과 분 기록의 제외 ID는 조회와 재계산을 위한 파생 상태로 둔다.
이 구분으로 GPS 품질 문제와 사용자의 편집 의도를 각각 보존하며, 제외 취소 시 원시
좌표에서 같은 파이프라인을 다시 실행할 수 있다.
...
`SessionGuard`는 최근 신뢰 선분을 시간순 deque로 보관한다. 각 항목은 시작 시각,
종료 시각, 계산 속도, 고속 여부를 가진다. 새 위치를 관찰할 때 최신 위치 시각보다
120초 이전인 부분을 버리고, 창 경계에 걸친 선분은 겹치는 시간만 합산한다. 합계가
60초에 도달하면 `highSpeedWarning`을 한 번 반환하고 경고 latch를 잠근다.
...
범위 시작은 첫 분 기록의 실제 세션 내 시작 시각, 범위 끝은 마지막 분 기록의 실제
끝 시각으로 정규화한다.
...
`LocationSample`에는 nullable `userExclusionId`를 추가한다. 기존 `isFilteredOut`은
자동 GPS 필터 결과만 뜻한다. `MinuteWindow`에도 nullable `userExclusionId`와
`isUserExcluded` getter를 추가한다. `SegmentMerger`는 제외 ID가 같은 연속 분만 하나의
제외 segment로 합치며 서로 다른 편집 레코드는 합치지 않는다.

### 3.3 재계산기

`SessionPipeline`에 완료 기록 재계산 경로를 추가한다. 입력은 완료 세션, 원시 샘플,
전체 제외 범위, 기존 분 기록의 사용자 메타데이터다. 결과는 다음을 포함한다.

- 자동 필터를 다시 적용한 전체 샘플과 각 샘플의 `userExclusionId`
- 제외 분을 포함해 전체 기록 시간을 덮는 새 `MinuteWindow` 목록
- 제외 경계와 신뢰할 수 없는 GPS 공백에서 나눈 지도용 경로 조각 목록
- 제외 시간을 반영한 `SessionRollupResult`

파이프라인은 먼저 전체 원시 샘플에 현재 `SampleFilter`를 적용한다. 그다음 각 샘플이
속하는 제외 범위를 표시한다. 자동 필터 결과와 사용자 제외 상태는 어느 단계에서도
서로 덮어쓰지 않는다.
...
포함된 분은 현재 집계와 추론 파이프라인으로 다시 만든다. 기존 분 기록의 사용자 라벨,
메모, 사용자 확정, 장소 연결은 `window_start`를 키로 다시 결합해 사용자의 다른 편집을
잃지 않는다. 제외를 취소한 분도 원시 샘플에서 다시 계산한 뒤 같은 메타데이터를 복원한다.

세션 집계는 다음 규칙을 사용한다.

- `duration_s`는 `ended_at - started_at`에서 제외 범위 합계를 뺀다.
- `total_distance_m`은 자동 필터를 통과하고 사용자 제외가 아닌 좌표만 사용한다.
- 제외 범위의 양쪽 좌표는 인접 좌표로 취급하지 않는다.
- 기존 `trustedLocationGap`보다 큰 공백의 양쪽 좌표도 연결하지 않는다.
- `moving_time_s`와 `stationary_time_s`는 같은 보존 경로 조각 안의 인접 좌표만 사용한다.
- `avg_speed_mps`는 재계산 거리 나누기 재계산 이동 시간이다.
- `valid_sample_count`와 `median_accuracy_m`은 자동 필터를 통과하고 제외되지 않은
  샘플만 사용한다.
...
- 좌표와 시각이 유효하고 시간순이다.
- `SampleFilter`가 두 위치를 자동 제외하지 않았다.
...
2분만 시간순으로 다시 관찰하고 마지막에 한 번 평가한다. 2분보다 오래된 고속 이동은
버리며, 최신 2분 안에 고속 60초가 실제로 남아 있으면 복구 직후 한 번 경고한다.
프로세스가 종료되기 전에 이미 경고했는지는 영속화하지 않으므로 재실행 뒤 같은 최근
구간으로 한 번 더 경고할 수 있다. 세션당 경고 이력을 추가로 저장하는 복잡성보다
종료 누락을 다시 알리는 안전성을 우선한다.
...
시스템 알림으로 보낸다. 알림을 누르면 앱의 홈 화면을 열고 같은 경고와 두 동작을
표시한다. 고속 알림은 고유 종류와 ID를 사용하므로 기존 장시간 정지, 5시간 제한,
완료 알림을 취소하지 않는다. 사용자가 앱에서 계속 기록하거나 종료하면 고속 알림만
취소한다.
...
권한 요청, 표시, 종류별 취소를 구현한다. 알림 권한은 첫 산책 시작 때 한 번 요청한다.
권한이 거부되거나 실행 중 철회돼도 위치 기록과 앱 내 경고는 계속 동작한다.
...
현재 `RouteMap`의 단일 points 입력을 여러 경로 조각을 받는 입력으로 확장한다. 각
조각은 자동 필터를 통과하고 사용자 제외가 아닌 연속 좌표만 가진다. 다음 경우 새
조각을 시작한다.

- 두 좌표 사이에 사용자 제외 범위가 있다.
- 인접 좌표의 시각 차이가 `trustedLocationGap`보다 크다.
- 자동 필터로 인해 신뢰할 수 있는 연결을 만들 수 없다.

지도는 각 조각을 별도 polyline으로 그리며 조각의 끝과 다음 조각의 시작을 절대 잇지
않는다. 경로 재생도 같은 조각 정보를 사용한다. 제외 전 확인 상태에서는 기존 강조
색으로 대상 구간을 보여 줄 수 있지만, 저장 뒤 기본 지도와 내보낸 경로에서는 제외된
좌표를 사용하지 않는다.
...
`location_samples`와 `minute_windows`에는 각각 nullable
`user_exclusion_id TEXT REFERENCES route_exclusions(id) ON DELETE SET NULL` 열을 추가한다.
값이 null이면 산책에 포함되고, 값이 있으면 해당 사용자 제외 레코드에 속한다.
`location_samples.is_filtered_out`의 의미와 값은 바꾸지 않는다.

3에서 4로 올릴 때 먼저 `route_exclusions`를 만들고 두 열과 인덱스를 추가한다. 기존
행은 모두 null이므로 기존 기록과 집계가 변하지 않는다. 마이그레이션은 SQLite의 기존
...
1. 대상 완료 세션, 전체 원시 샘플, 기존 분 기록, 전체 제외 레코드를 같은 transaction
   executor로 읽는다.
2. 제외 시 새 `route_exclusions` 행을 추가하고, 복원 시 대상 행을 제거할 새 상태를
   메모리에서 구성한다.
3. `SessionPipeline`으로 샘플 파생 상태, 분 기록, 경로 조각, 세션 집계를 다시 계산한다.
4. `location_samples.user_exclusion_id`를 새 결과와 일치하도록 갱신한다.
5. 대상 세션의 `minute_windows`를 새 결과로 교체한다.
6. `sessions`의 거리, 유효 시간, 이동 시간, 정지 시간, 평균 속도, 유효 샘플 수,
   중앙 정확도를 갱신한다.
7. 제외 시 새 제외 행을 유지하고, 복원 시 참조가 사라진 대상 제외 행을 삭제한다.
...
전체 백업 스키마를 1에서 2로 올린다. 버전 2는 `route_exclusions` 테이블과 샘플 및 분
기록의 `user_exclusion_id`를 포함한다. 내보내기는 완료 세션에 속한 제외 레코드만
기록하며, 세션, 제외, 샘플, 장소, 분 기록의 외래 키 관계를 검증한다.

가져오기는 백업 버전 1과 2를 모두 지원한다.

- 버전 1에는 제외 테이블과 열이 없으므로 빈 제외 목록과 null 제외 ID를 합성한다.
- 버전 2는 제외 범위가 세션 안에 있고 서로 겹치지 않는지 검증한다.
- 샘플과 분 기록의 제외 ID가 같은 세션의 실제 제외 레코드를 가리키는지 검증한다.
...
NDJSON 단일 세션 내보내기에도 제외 레코드와 각 샘플 및 분 기록의 제외 ID를 포함한다.
...
- 고속 guard가 판단할 만큼 신뢰 위치가 부족하면 경고하지 않는다. 불확실한 데이터를
  이동으로 단정하지 않는다.
- 고속 경고와 정지 경고가 동시에 가능하면 최신 실제 상태에 맞는 하나의 앱 내 경고만
  표시한다. 30분 정지 제한과 5시간 전체 제한은 기존 우선순위대로 자동 종료한다.
...
- 제외 트랜잭션 실패 시 메모리 provider를 무효화하지 않는다. 사용자는 변경 전 지도와
  통계를 계속 보며 재시도할 수 있다.
- 앱이 트랜잭션 도중 종료돼도 SQLite rollback으로 제외 레코드, 샘플, 분 기록, 세션
  집계가 이전 일관된 상태로 돌아간다.
