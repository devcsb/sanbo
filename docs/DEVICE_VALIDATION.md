...
- 2026-08-25 커밋 `7a35094`에서 무작위 순서 seed `91237`로 Flutter 테스트 356개를 통과시켰다. 이어 Android emulator `emulator-5554`의 알림 거부·화면 잠금 provider smoke는 68.3895m, 유효 샘플 6개와 종료 후 provider 해제를 통과했고, 고속 cold tap의 앱 내부 `기록 종료`, 차량 구간 제외, 강제 종료 뒤 제외 상태 유지와 복원은 970.3855m, 유효 샘플 13개로 통과했다. iPhone 17 Pro simulator는 화면 전원을 20초 뒤 끈 실제 Core Location 고속 E2E와 fresh process notification channel probe를 통과했다. Android logcat과 iOS simulator 로그에서 앱 fatal exception 또는 crash 흔적은 발견되지 않았다. 이 실행은 emulator와 simulator 증거이며 물리 기기 판정으로 승격하지 않는다.
...
- 같은 날 위치 권한 철회 cold recovery smoke 첫 재실행에서 복구 직후 Android 알림 권한 대화상자가 다시 나타나는 환경 동작을 발견했다. 앱 provider는 이미 재개됐지만 대화상자가 `기록 중` UI 검사를 가렸고, 권한 철회 분기에도 공통 대화상자 닫기를 적용한 뒤 `SANBO_ANDROID_CLEAR_DATA=1 SANBO_ANDROID_NOTIFICATION_PERMISSION=deny SANBO_ANDROID_REVOKE_LOCATION_AFTER_START=1`을 다시 실행해 18.7843m, 유효 샘플 4개, 종료 후 provider 해제를 통과했다. 해당 순서를 문서 계약 회귀로 고정했으며, provider 장애 smoke는 63.1872m와 유효 샘플 6개, 고속 cold tap과 차량 구간 제외, 재시작 보존 smoke는 973.0409m와 유효 샘플 14개로 다시 통과했다. iOS simulator 화면 전원 끄기 고속 E2E와 fresh notification channel probe도 통과했다. 모두 simulator와 emulator 증거이며 물리 기기 판정으로 승격하지 않는다.
- 같은 날 추가 검증에서 무작위 순서 seed `271828`과 `8675309`의 Flutter 테스트 356개, `flutter analyze`, quality loop를 통과했다. Android emulator `emulator-5554`의 알림 허용 warm tap과 `계속 기록`은 970.3855m와 유효 샘플 13개로 통과했고, 알림 거부·화면 잠금 provider smoke는 21.9251m와 유효 샘플 5개, 종료 후 provider 해제로 통과했다. iPhone 17 Pro simulator의 화면 전원 끄기 고속 Core Location E2E와 fresh notification channel probe, 양 플랫폼의 종료 후 crash 로그 점검도 통과했다. 이 결과는 emulator와 simulator 증거이며 물리 기기 판정으로 승격하지 않는다.
...
- 같은 simulator에서 `flutter test --no-pub -d 96749A10-F3A8-4C98-87EE-79A8EE439BDA integration_test/app_e2e_test.dart`를 실행했다. UI 세션 저장과 고속 경고 확인, `계속 기록`, 차량 이동 구간 제외, 총거리 감소, `제외 취소` 뒤 원래 거리 복원을 2개 테스트로 통과했다. synthetic 위치 주입과 simulator 결과는 실제 iPhone의 GPS와 절전 정책 판정으로 승격하지 않는다.
