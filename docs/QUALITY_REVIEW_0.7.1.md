...
2. 실제 잠금 화면의 20/30분 정지 및 4시간 45분/5시간 알림·자동 종료
...
## 0.7.2-dev 세션 자동 종료 정책 후속 검토

전체 세션 보호 한도를 3시간에서 5시간으로 확대하고, 종료 예고를 4시간 45분으로
이동했다. 도메인 기본 정책, 컨트롤러의 앱/OS 알림·완료 문구, 경계 시각 테스트와
PRD/TRD/README를 같은 기준으로 갱신했다. 한 장소 정지 20분 경고·30분 자동 종료와
자동 저장 실패 시 복구 가능한 상태는 유지한다.

자동 종료 시각은 합성 흐름 테스트로 4시간 45분 경고와 5시간 저장·종료를 검증했다.
실제 잠금 화면에서 제조사 절전이 5시간 유지·알림·자동 저장에 미치는 영향은
`DEVICE_VALIDATION.md`의 장시간 장치 검증으로 별도 확인해야 하며, 이 변경만으로
실기기 측정 완료를 주장하지 않는다.
